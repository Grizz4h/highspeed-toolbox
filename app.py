...
CONFIG_PATH = Path("canon_time_config.json")
...
def render_clickable_month(cfg, season: int, year: int, month: int, events):
    """Clickable month grid: clicking a day sets session_state['picked_date']."""
...
with st.sidebar:
    ...
    if st.button("♻️ Stand zurücksetzen"):
        delete_state()
        st.session_state["ui_state"] = load_state()
...
with left:
    ...
    with colB:
        if st.button("➕ Content → Kalender", key="btn_add_content"):
            try:
                ...
                meta = {"season": int(season), "matchday": int(matchday), "content_type": content_type, "offset": int(offset)}
    ...
    with st.expander("Neue Season anlegen", expanded=False):
        ...
        new_start = st.date_input("Startdatum (Matchday 1)", value=cfg.season_start[available_seasons[0]], key="new_season_start")
...
with right:
    ...
    if todays:
        ...
        for ev in todays:
            with st.container(border=True):
                ...
                # delete button (no extra state changes!)
    ...
    # Free entry add: use a form with clear_on_submit (no session_state writebacks)
